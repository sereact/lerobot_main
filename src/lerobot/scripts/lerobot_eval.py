...
def rollout(
    ...
    while not np.all(done) and step < max_steps:
        ...
        # VectorEnv wants shape (B, A); if (A,) add batch dim
        if action_np.ndim == 1:
...
def eval_main(cfg: EvalPipelineConfig):
    ...
    envs = make_env(cfg.env, n_envs=cfg.eval.batch_size, use_async_envs=cfg.eval.use_async_envs, enable_depth=cfg.env.enable_depth, action_type=cfg.env.action_type)
