    ...
    for task_id in tqdm.tqdm(range(num_tasks_in_suite)):
        ...
        for i in range(len(orig_data.keys())):
            ...
            for _, action in enumerate(orig_actions):
                ...
                # if num_replays == 0 and _ == 0:  # only visualize first frame of first episode
                #     import matplotlib.pyplot as plt

                #     fig, axs = plt.subplots(1, 3, figsize=(12, 4))
                #     axs[0].imshow(obs["agentview_image"].astype(np.uint8))
                #     axs[0].set_title("Raw Agentview")
                #     axs[1].imshow(agentview_images[-1].astype(np.uint8))
                #     axs[1].set_title("Flipped Agentview")
                #     axs[2].imshow(eye_in_hand_images[-1].astype(np.uint8))
                #     axs[2].set_title("Flipped Eye-in-Hand")
                #     for ax in axs:
                #         ax.axis("off")
                #     plt.tight_layout()
                ...
                #     save_path = os.path.join(script_dir, "first_frame_debug.png")
                #     plt.savefig(save_path, dpi=150)
                #     plt.close(fig)
                #     print(f"[debug] Saved first frame visualization at: {save_path}")
            ...
            if not done: 
                print(f"Episode {i} did not finish. Skipping...")
                continue
            num_replays += 1
